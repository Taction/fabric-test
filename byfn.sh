...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_CA} -f ${COMPOSE_FILE_TLSCA}"
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.example.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.example.com/ca && ls *_sk)
    export BYFN_TLSCA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.example.com/tlsca && ls *_sk)
    export BYFN_TLSCA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.example.com/tlsca && ls *_sk)
  fi
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_TLSCA -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
...
COMPOSE_FILE_TLSCA=docker-compose-tlsca.yaml
#
