...
createSysConfigUpdate() {
  ...
  set -x
  configtxlator proto_encode --input "${ORIGINAL}" --type common.Config >original_sysconfig.pb
  configtxlator proto_encode --input "${MODIFIED}" --type common.Config >modified_sysconfig.pb
  configtxlator compute_update --channel_id "${CHANNEL}" --original original_sysconfig.pb --updated modified_sysconfig.pb >sysconfig_update.pb
  configtxlator proto_decode --input sysconfig_update.pb --type common.ConfigUpdate >sysconfig_update.json
  echo '{"payload":{"header":{"channel_header":{"channel_id":"'$CHANNEL'", "type":2}},"data":{"config_update":'$(cat sysconfig_update.json)'}}}' | jq . >config_update_sys_in_envelope.json
  configtxlator proto_encode --input config_update_sys_in_envelope.json --type common.Envelope >"${OUTPUT}"
  set +x
}
